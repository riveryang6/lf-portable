...
启动器或便携运行时修复的完成声明必须以同一修订的真实产物通过两项实测为前提：从 `CODEX_USB` U 盘版本实际启动并打开桌面，以及在 Windows Sandbox 中实际启动并打开桌面。未挂载卷标为 `CODEX_USB` 的卷时，跳过 U 盘部署及清理前后两次真机验收，不阻塞完成和发布，但交付说明必须明确记录该项因未挂载而未执行。已挂载时，任一项未执行、失败，或仅有静态检查、本机临时编译通过，均不得声称完成；应继续迭代，或明确报告尚未完成。该条是人工验收要求，不新增项目内自动门禁、checkpoint、hash、manifest 或持久化证据文件。

真实 U 盘验收前必须退出本任务启动的 LF Portable 进程，清除本任务在固定盘创建的 LF 运行状态和临时缓存，然后只运行 `CODEX_USB` 根目录的 `CodexPortable.exe`。桌面主进程、程序文件和运行库必须来自该便携根目录，不得依赖预装、预导入或跨次保留的本机 LF 程序镜像、包缓存或其他本机状态。任务收尾清理完成后必须从同一 U 盘产物再重复一次启动验收；清理前的成功不能作为最终完成依据。
...
便携启动器和它启动的 Codex Desktop 必须沿用调用者的 Windows 令牌；内嵌 manifest 必须保持 `requestedExecutionLevel=asInvoker` 和 `uiAccess=false`。正常启动、交接和便携运行准备不得使用 `requireAdministrator`、`runas` 或其他隐式提权路径。UAC 是否显示提示不改变这条约束：高完整性进程会破坏默认 userdata 权限上下文，并可能让 Computer Use 的自动点击受到 UIPI 拒绝。发布前必须从最终二进制检查 manifest，并在 U 盘和 Sandbox 实测中确认启动器、桌面进程和自动化操作仍处于预期的调用者权限上下文。
...
每次任务完成、发布完成或明确中断时，先清理本次明确产生且不再作为当前源码、运行输入或待交付物的本机残留，再交付结果。清理范围包括任务专用的 `build/`、`release/`、`package/` 暂存目录，已上传发布后的本地 ZIP 副本，临时截图、Sandbox `.wsb`/runner、可选本机会话缓存及其他任务创建的本机 LF 状态，以及由脚本变量误写入仓库根目录的临时目录。
