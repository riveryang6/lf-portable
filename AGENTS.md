...
**未经用户授权不得发布。** 创建版本 tag、推送到远端、创建/编辑 GitHub Release、上传发布资产，以及把新版本同步到 U 盘或任何外部位置，都必须先向用户陈述本改动内容与验证证据，并取得用户在当前任务内的明确授权后才能执行；不得把“仓库规则要求每次改动都发布”当作自动发布的理由。本地构建、真实启动/运行验证与修复不受此限，可先行完成并汇报。若用户此前只授权到某一步（例如只做本地编辑），则发布动作一律停在该步之前。
