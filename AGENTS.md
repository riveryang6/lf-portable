...
**GitHub Release 发布需当次授权。** 创建/编辑 GitHub Release 与上传发布资产，必须先向用户陈述改动内容与验证证据并取得当次明确授权；不得把任何仓库规则当作绕过授权的理由。若用户此前只授权到某一步（例如只做本地编辑），发布动作一律停在该步之前。

**本地发布与同步可自主推进。** 组装 `release/<ver>`、同步提交 release 目录与 U 盘/dist 根入口、更新版本号、提交变更、创建并推送版本 tag、远端 commit push，均可按任务内规则直接执行，无需逐次询问。**不保留旧版本**：每次发布新版本后清理旧的 `release/` 目录/副本与过时暂存，只保留当前版本产物；不要在仓库或磁盘上堆积历史版本文件。

**修复完成以交付物替换为准（含 U 盘）。** 宣称某问题“已修复/已闭环”之前，必须先核实并如实说明交付物状态：只修到源码、固定盘或临时构建，而 U 盘根目录或 `dist/` 正式单文件仍是旧版时，不得声称该问题已向用户交付修复；未替换的卷必须明确列为未完成项（注明卷符、旧版本号/旧文件大小）。凡修复会改变交付行为，在用户授权范围内，必须把新组装单文件替换到被点名的 U 盘根与 `dist/CodexPortable.exe`，并从该卷根做一次真实启动/运行验证后才可陈述“已修复/已同步”；替换动作本身仍受上一条“未经用户授权不得发布/同步”约束。



凡会改变交付行为或交付内容的源码、构建脚本、发布脚本、规则或文档修改，在验证成功后都必须在同一任务内完成一个新的版本发布；仅完成本地构建、启动或测试不算任务完成，除非用户明确要求只做本地编辑。闭环顺序为：从当前主仓库和明确指定的输入重新组装正式架构 EXE，确认输入角色和可启动性，更新版本号，提交变更，创建版本 tag，推送提交和 tag，同步 U 盘/dist，并清理旧版本产物；`publish-release.py` 只在用户当次授权 GitHub Release 发布时运行。发布操作失败时必须继续排查并修复可处理的输入或流程问题；只有确实缺少必要输入或外部权限且安全替代路径均已检查后，才能报告具体阻塞原因。拒绝裸 bootstrapper 只是纠正输入，不能代替后续组装、发布或完成结论。本闭环不新增 checkpoint、收据、hash、manifest 比对或其他自定义状态文件。

**未经真实启动/运行验证不得发布。** 新组装的架构单文件在提交版本号、创建 tag 或发布前，必须先从该单文件所在卷/目录真正启动一次（不能用轻量 fixture、仅编译、目录解析或界面单帧替代），完成首次展开与载荷准备，确认 Codex Desktop 主进程（程序文件与运行库来自该便携根）实际出现并进入后续可用阶段；同一任务里任何会改变 payload 准备行为的代码改动后都要重跑该验证。启动验证失败（含首次展开、品牌/载荷准备、运行交接任一环节报错）时，必须把启动修复到通过才能 tag 与发布，不得以“版本号相同/构建成功/已上传”搪塞。
...
**不要求用户配合做任何你能自己完成的操作。** 凡可由本会话自行执行或自动化的步骤（关闭由本任务/便携启动的进程与窗口、同步/清理、启动与验收、弹窗点击、日志与截图取证等），必须先自行完成并给出证据；不得以“请你自己关掉/请你在真机确认”等形式把可自动化步骤推回给用户。只有经过尝试仍确属本会话不可及（如窗口不在本会话可见且无法通过 CDP/枚举替代）的项，才以最小清单留给用户，并说明已做尝试与证据。
